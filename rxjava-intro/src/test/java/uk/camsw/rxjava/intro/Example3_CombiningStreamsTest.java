...
public class Example3_CombiningStreamsTest {
    ...
    public void merge() {
        ...
        // If my callbacks become observables then I can begin to deal with them in different ways, without really usage specific writing code
    ...
    @Test
    public void concat() {
        // How about if I want to consume 1 stream until such time as it completes, then seamlessly switch over to another
    ...
    public void zipWithDifferentFrequencyProducers() {
        ...
        producer2 = Producer.builder()
                .withFrequency(Duration.ofMillis(100))      // Stream 2 produces 10 times quicker than stream 1
                .startFrom(100)
                .build();
        ...
        // You can see that even though one side is producing much quicker than the other, it is still giving us our correctly 'paired' items
    ...
    public void zipWithBackpressureHandling() {
        ...
        // eventually give up and request you provide a backpressure strategy
        producer2 = Producer.builder()
...
//                        .onBackpressureLatest()
        ...
        // Because we chose 'onBackpressureLatest', once we have hit the limit of our buffer, we conflate away subsequent events
        // When our buffer is drained, we'll start taking events from stream 2
    ...
    public void combineLatest() {
        ...
        // You should see a value from stream 1 being combined multiple times with different values from the faster producing stream 2
        Uninterruptibles.sleepUninterruptibly(3, TimeUnit.SECONDS);
    }
...
//                .cache(1)
        ...
        // The takeUntil causes the deprecated stream to complete (it's contract being 'take until this given stream emits')
        // The completion of the deprecatedStream causes the concat operation to become effective (it's contract being 'switch to this stream following my normal termination')
        // We are now subscribed to the new, live stream and are no longer consuming from the deprecated stream
        // NOTE: We take(5) just to prove our composability, we want 5 items and we don't care where they came from because we have coerced 2 streams into one

        // Is it perfect?  Nope, look carefully and you will see that we missed the first emission from the new stream, it triggered the concat but never emitted the trigger
        // How can we solve it?
        //    - Uncomment the cache(1) line on the new stream.  Cache is an operator that will cache 'n' emissions, replaying them to late subscribers
