...
public class Example5_ManagingResourcesTest {
    ...
    public void before() {
        ...
        // Imagine that it produces values by polling an external system that hasn't yet discovered event driven architectures
        ...
        producer = Observable.create(observer -> {
            ...
            // What should we do with the subscription, well, we tell the observer about it via the (hideously misnamed) add method
            observer.add(schedulerSubscription);

            // What does it mean?  That when the subscriber unsubscribes from this stream, we also want it to unsubscribe any other subscriptions we have given it
            // Let's add another Subscription to the observer, just for fun
            observer.add(Subscriptions.create(() -> System.out.println("I can associate whatever tear-down behaviour I need by adding a subscription to the observer")));
        });
    ...
    public void explicitUnsubscribe() {
        ...
        // After 3 seconds we have explicitly unsubscribed.  We can see our output from the custom subscription, it's fair to assume that the scheduler has also been stopped
    }
    ...
    public void unsubscribeDueToCompletion() {
        ...
        producer.take(2)
                .subscribe(n -> System.out.println("n = " + n), System.err::println, () -> System.out.println("I Completed"));
        ...
        // Well, I didn't subscribe directly to my producer, rather I subscribed to the observable returned from the take(2) operation
    ...
    public void multipleSubscribers() {
        // In the real world it's quite likely that more than one subscriber is interested in the values emitted from our legacy producer app
    ...
    public void sharingResources() {
        // The above may be exactly what you want, but the owners of the legacy system may not be impressed when your 10,000 subscribers start polling it simultaneously for the
        // same data
        ...
        //  - Both subscribers (when subscribed) received the same values on the stream
        //  - Subscriber 2 continues to receive values even when subscriber 1 unsubscribes
        //  - When subscriber 2 unsubscribes however, we can see the polling scheduler being taken down
        // We don't see it here (to avoid clutter), but were we to subscribe again, you would see the poller being scheduled again

        // It may not be entirely obvious how this is working, but hopefully you can see the behaviour, which can be summarised
        //  - publish()   - An operator that multiplexes the stream (effectively this stops the lifecycle of the source being controlled by subscribe/ unsubscribe activity)
        //  - refCount() - An operator that manages a multiplexed (published) stream's subscription, based on the number of subscribers it has subscribed to itself
        //               - Additionally refCount emits every event it receives to all of it's subscribers

        // So, refcount will 'connect' publish upon receiving it's first subscriber
        // A subsequent subscribe will increment the 'refcount', the new subscriber will begin receiving events also
        // When a subscriber unsubscribes, the refcount is decremented
        // When the refcount is zero, the refCount operator unsubscribes from the 'published' stream, which unsubscribes from the underlying producer and tears down the scheduler

        // That's quite a lot to take in, but it's given us a really cool way of creating expensive resources on demand and only keeping them for as long as we need them
    }
